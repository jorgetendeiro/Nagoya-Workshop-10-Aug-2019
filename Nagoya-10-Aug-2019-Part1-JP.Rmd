...
nocite: | 
  @cuddy2018, @wasserstein2016, @klein2014, @trafimow2015, @heathers2018, @spreckelsen2018, @wilkinson1999, @ikeda2019
---
...
- データの偽造または加工.
- 意図的であり、故意的でないもの.
- 全科学的研究が <span style="color:red">疑われる</span>.
...
# Questionable research practices （好ましくない研究行為）{.section}
...
- 実際に*許容される範囲内の* 研究行為とその悪用も含む.
- QRPs（好ましくない研究行為）に関する問題点:<br>
...
- いくつかの条件を取り除く.
- <span style="color:red">覗き見</span>: 逐次試験 --- 結果を見ながら決める:
    * $p > .05$: もっとデータを集める.
    * $p < .05$: データ収集を止める.
- $p<.05$の結果のみ報告する.
...
- <span style="color:red">HARKing</span> [@kerr1998]: 探索的に得られた結果を研究課題に変えること.<br>
(@bem2004,  2011年に出版された予知に関する有名な論文の著者はこの手法を, *正当防衛しようと*したが...)
- ...
...
<span style="color:red">これは QRPing　（好ましくない研究行為）である.</span>

## Wansink教授はどうなったか?
- かなり批判され, 彼の研究は精査された [e.g., @vanderzee2017].
- 100以上におよぶ間違い が４セットの論文から発見された...
...
[もちろん](https://www.westernalliance.org.au/2018/05/restoring-trust-in-science-what-are-questionable-research-practices).
...
## 研究の事前登録(preregistration)をすればQRPs（好ましくない研究行為）はなくなるのか?...
残念ながら, （まだ）そうはならない.
...
#*なぜ*事前登録だけではQRP（好ましくない研究行為）はなくならないのか?... {.section}

## なぜQRP（好ましくない研究行為）は蔓延しているのか?
それはインセンティブ（動機）と深く関係がある [@nosek2012; @schonbrodt2015].

- "Publish or perish"（論文を出版するか滅びるか）:<br>
より多くの論文を、より権威あるジャーナルに出版しなければならないという現実.
    - ジャーナルは提出されたうちのごく一部しか出版しない.
...
しかし, <span style="color:red">忘れてはならないのは</span>, <span style="color:green">研究者の最善の意図を持ってしても</span>好ましくない研究行為は起こり得るということである.

- 不十分な統計科目の教育 (そう、統計学者はこの点を理解すべきであると思います!...).
- 各分野にある永続的な伝統.

# (I)reproducibility (再現性) {.section}
...
## 結果
**全ての** 文は間違っています <span style="color:red">incorrect</span>.
...
- しかし、信頼区間は推定されたパラメーターの特性を直接的に示すものでは(<span style="color:red">全く</span>!)ない！
<br><br>
...
## 結果
**全ての**文は <span style="color:red">間違っています</span>.
...
完全性のために述べておくと、全員がHoekstraの研究[@garcia-perez2016; @miller2016; see also a reply by @morey2016]を支持している訳ではない.
...
# 統計協会(statistical associations)はどうアドバイスしているのか? {.section}

## Wilkinson Task Force 1999
多くのアドバイスの一部抜粋、

- $p$ー値ばかり気にするな.
- 効果サイズ(effect sizes)を報告する.
- 検出力分析(power analyses)を報告する.
...
3. 科学的結論、ビジネスまたは政策的決断を $p$-値が特定の値を越えるかどうかのみによって決めるべきではない.
4.適切な推定をするためには、全て報告し、透明でなければならない.
5. $p$-値または統計的有意性は効果(effect size)の度合いや結果の重要性を示す指標ではない.
...
- 勿論、いじめるために(特にしっかりした理由なく)批判すべきではない(例) Wansink).
- 私見ですが、時に批判することに<span style="color:red">無我夢中になりすぎ</span>ている場合もある.
<br><br>
